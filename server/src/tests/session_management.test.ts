...
async function runSessionTests() {
  ...
  const folderId = await database.saveFolder('Architektura KAN', '#06b6d4');
  ...
  const sessAId = await database.createSession('Nowa rozmowa', folderId);
  ...
  await database.updateSession(sessAId, { title: heuristicTitle });

  const sessionA = await database.getSession(sessAId);
  ...
  const sessBId = await database.createSession('Sesja B - Pamięć EWC');

  await database.saveMessage('user', 'Pytanie w sesji A', 10, 'gpt-4o', undefined, sessAId);
  await database.saveMessage('assistant', 'Odpowiedź w sesji A', 20, 'gpt-4o', undefined, sessAId);

  await database.saveMessage('user', 'Pytanie w sesji B o EWC', 15, 'gpt-4o', undefined, sessBId);

  const msgsA = await database.getRecentMessages(10, sessAId);
  const msgsB = await database.getRecentMessages(10, sessBId);
  ...
  await database.updateSession(sessBId, { is_pinned: true });

  const sessionsAll = await database.getSessions({ includeArchived: false });
  ...
  await database.updateSession(sessAId, { is_archived: true });

  const activeSessions = await database.getSessions({ includeArchived: false });
  ...
  const archivedSessions = await database.getSessions({ includeArchived: true });
  ...
  await database.updateSession(sessAId, { is_archived: false });
  ...
  const searchResultsTitle = await database.searchAllSessions('splajny');
  ...
  const searchResultsContent = await database.searchAllSessions('EWC');
  ...
  const resolvedContexts = await sessionResolver.resolveSessionContexts(extractedIds, 'jak połączyć KAN z EWC?');
  assert.equal(resolvedContexts.length, 2);
  assert.ok(resolvedContexts[0].contextText.includes('ODWOŁANIE DO POWIĄZANEJ SESJI'));
  // Weryfikacja budżetu tokenów (maksymalnie 400 tokenów łącznie)
  const totalTokensEstimated = resolvedContexts.reduce((sum, r) => sum + Math.ceil(r.contextText.length / 3.5), 0);
  assert.ok(totalTokensEstimated <= 400, `Całkowity kontekst (${totalTokensEstimated} tokenów) nie może przekroczyć 400`);
  console.log(`✓ Pomyślnie rozwiązano i skompilowano kontekst zewnętrznych sesji (tokeny: ~${totalTokensEstimated} <= 400).`);

  // 8. Test Kaskadowego Usuwania Sesji
  console.log('Test 8: Kaskadowe usuwanie sesji i wiadomości...');
  await database.deleteSession(sessAId);
  const deletedSession = await database.getSession(sessAId);
  assert.ok(!deletedSession, 'Sesja A powinna być usunięta');
  const remainingMsgsA = await database.getRecentMessages(10, sessAId);
  ...
  const deletedFolder = (await database.getFolders()).find((f: any) => f.id === folderId);
  assert.ok(!deletedFolder, 'Folder powinien zostać usunięty');
